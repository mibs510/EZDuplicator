...
from EZDuplicator.lib.weresync.exception import CopyError
    ...
    def install_bootloader(self, source_mnt, target_mnt, copier,
        ...
        if root_partition is None and boot_partition is None:
            part = plugins.search_for_boot_part(target_mnt, copier.target,
                                                ...
                                                excluded_partitions)
            if part is None:
                raise CopyError("Could not find partition with 'boot' folder on device {0}".
                                format(copier.target.device))
