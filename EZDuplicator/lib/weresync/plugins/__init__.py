...
from EZDuplicator.lib.weresync.exception import DeviceError
...
def search_for_boot_part(
    ...
    if exlcuded_partitions is None:
        exlcuded_partitions = []
    for i in target_manager.get_partitions():
        ...
        try:
            ...
            logging.warning("Could not mount partition {0}. Assumed to not be the partition {1} is on.".
                            format(i, search_folder))
            logging.error(ex)
        # finally:
        # try:
        # if mounted_here:
