...
from EZDuplicator.lib.weresync.exception import CopyError, DeviceError
from EZDuplicator.lib.weresync.plugins import IBootPlugin
...
class GrubPlugin(IBootPlugin):
    ...
    def install_bootloader(self, source_mnt, target_mnt, copier,
        ...
        if efi_partition is not None:
            import lib.weresync.plugins.weresync_uuid_copy as uuc
        ...
        if root_partition is None and boot_partition is None:
            ...
            for i in copier.target.get_partitions():
                try:
                    ...
                    logging.warning("Could not mount partition {0}. Assumed to not be the partition grub is on.".
                                    format(i))
                    logging.debug("Error info:\n", exc_info=sys.exc_info())
            else:  # No partition found
                raise CopyError("Could not find partition with "
                                "'boot/grub' folder on device {0}".format(copier.target.device))
        ...
        try:
            ...
            logging.debug("Grub command: " + " ".join(grub_command))
            ...
            logging.debug("Consider running update-grub on your backup. WereSync copies can sometimes fail to capture "
                          "all the nuances of a complex system.")
