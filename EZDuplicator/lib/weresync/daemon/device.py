...
from EZDuplicator.lib.weresync.exception import DeviceError, PluginNotFoundError

MOUNT_POINT = "/mnt"

SUPPORTED_FILESYSTEM_TYPES = ["swap", "ef"]
for mkfs in glob.glob("/*/mkfs.*"):
    utils = mkfs.split("/")[2]
    file_system = utils.split(".")[1]
    SUPPORTED_FILESYSTEM_TYPES.append(file_system)

SUPPORTED_PARTCLONE_FILESYSTEM_TYPES = []
for partclone in glob.glob("/*/partclone.*"):
    utils = partclone.split("/")[2]
    file_system = utils.split(".")[1]
    SUPPORTED_PARTCLONE_FILESYSTEM_TYPES.append(file_system)
...
class DeviceManager:
    ...
    :py:class:`~lib.weresync.exception.DeviceError`
    ...
    def __init__(self, device, mnt=MOUNT_POINT, partition_mask="{0}{1}"):
    ...
    def get_partitions(self):
        ...
        if exit_code != 0:
            if "unrecognised disk label" in str(error):
                return partitions
            else:
                raise EZDuplicator.lib.weresync.exception.DeviceError(self.device, "Non-zero exit code",
                                                                      str(error, "utf-8"))
    ...
    def mount_point(self, partition_num):
        ...
        mount_point = str(output, "utf-8").split("\n")
        exit_code = findprocess.returncode
        if exit_code == 0:
            return mount_point[1].strip().split()[0]
        elif exit_code == 1:
            return None
    ...
    def mount_partition(self, partition_num, mount_loc):
        ...
        :raises: :py:class:`~lib.weresync.exception.DeviceError` if there is an
                 error mounting the device.
        """
        sleep(.2)
        mount_proc = subprocess.Popen(["mount", self.part_mask.format(self.device, partition_num), mount_loc],
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE)
        ...
        if exit_code != 0:
            raise EZDuplicator.lib.weresync.exception.DeviceError(self.device, 
                                                                  "Non-zero exit code. Partition Number: {0}".
                                                                  format(partition_num), str(error, "utf-8"))
    ...
    def unmount_partition(self, partition_num):
        ...
        :raises: :py:class:`~lib.weresync.exception.DeviceError` if the partition
        ...
        unmount_proc = subprocess.Popen(
            ...
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE)
        output, error = unmount_proc.communicate()
        exit_code = unmount_proc.returncode
        if exit_code != 0:
            logging.exception("{}: {}".format(self.device, str(error, "utf-8")))
    ...
    def _get_blkid_info(self, partition_num, info_name):
        ...
        if proc.returncode != 0:
            raise DeviceError(self.device,
                              ...
                              str(partition_num), str(error, "utf-8"))
    ...
    def get_partition_table_type(self):
        ...
        for table_type in SUPPORTED_PARTITION_TABLE_TYPES:
            ...
            raise EZDuplicator.lib.weresync.exception.UnsupportedDeviceError(
                "Partition table "
    ...
    def _get_general_info(self, partition_num):
        ...
        Filesystem (/dev/sda or such), Size in 1K-blocks, Used, Available,
        ...
        try:
            if self.mount_point(partition_num) is None:
                self.mount_partition(partition_num, self.mnt)
                mounted_here = True

            proc_formal = subprocess.Popen(
                ["df"], stdout=subprocess.PIPE)
            proc = subprocess.Popen(
            ...
            if exit_code >= 2:  # grep returns 2 if an error occurs
                raise EZDuplicator.lib.weresync.exception.DeviceError(
                    self.device, "Error running grep.",
                    str(error, "utf-8") + str(exit_code))
            elif exit_code == 1:
                raise EZDuplicator.lib.weresync.exception.DeviceError(self.device,
                                                                      "No grep line read", None)
            data = [x for x in str(output, "utf-8").split() if x != ""]
            return data
    ...
    def get_partition_used(self, partition_num):
        """Returns the space available in a drive in 1K blocks

        :param partition_num: the number of the partition to check"""

        partition_file_system = self.get_partition_file_system(partition_num)
        if partition_file_system is None or partition_file_system == "swap":
            return 0
    ...
    def get_partition_size(self, partition_num):
        ...
        if table_type == "gpt":
            ...
            device = self.part_mask.format(self.device, partition_num)
            """ Give the OS a bit of time to recognize the new partition table otherwise sfdisk will complain that 
            /dev/sdXY does not exist. """
            sleep(.1)
            proc = subprocess.Popen("sfdisk -s {}".format(device),
                                    stdout=subprocess.PIPE,
                                    stderr=subprocess.STDOUT, shell=True)
            output, error = proc.communicate()
            if proc.returncode != 0:
                raise EZDuplicator.lib.weresync.exception.DeviceError(self.device,
                                                                      "Error getting partition size for partition {0}".
                                                                      format(partition_num), str(error, "utf-8"))
            return int(output)
        else:
    ...
    def get_partition_file_system(self, part_num):
        ...
        if proc.returncode != 0 and proc.returncode != 2:
            raise EZDuplicator.lib.weresync.exception.DeviceError(
                self.part_mask.format(self.device, part_num),
                "Error getting partition file system type.", error)
    ...
    def set_partition_file_system(self, part_num, system_type):
        ...
        try:
            ...
            if system_type == "swap":
                ...
                if file_system == "ntfs":
                    args = ["mkfs", "-t", system_type, "-Q", self.part_mask.format(self.device, part_num)]
                else:
                    args = ["mkfs", "-t", system_type, self.part_mask.format(self.device, part_num)]
...
class DeviceCopier:
    ...
    def __init__(self, source, target, mnt_source, mnt_target, lvm_source=None, lvm_target=None):
        self.source = source if isinstance(
            source, DeviceManager) else DeviceManager(source, mnt_source)
        self.target = target if isinstance(
            target, DeviceManager) else DeviceManager(target, mnt_target)
        self.mnt_source = mnt_source
        self.mnt_target = mnt_target
    ...
    def get_uuid_dict(self):
        ...
        if self.uuid_dict is None:
            ...
            logging.debug("UUID Dict: " + str(uuids))
            return uuids
        else:
            return self.uuid_dict

    def _transfer_gpt(self, difference, resize=True):
        ...
        for i in reversed(partitions):
            ...
            except EZDuplicator.lib.weresync.exception.DeviceError:
                part_used = drive_size
            space = int(part_alignment * math.floor(
                (drive_size - part_used) / part_alignment))
            part_size = None
            if space > 0 and difference > 0 and resize:
        ...
        clear_process = subprocess.Popen(
            ...
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE)
        clear_output, error = clear_process.communicate()
        if clear_process.returncode != 0:
            logging.debug("Clear Process error code: {0}".format(
        ...
        final_proc = subprocess.Popen(
            ["sgdisk", self.target.device, "-G"], stderr=subprocess.PIPE, stdout=subprocess.PIPE)
    ...
    def _transfer_msdos(self, difference, margin=5, resize=True):
        ...
        if table_proc.returncode != 0:
            raise EZDuplicator.lib.weresync.exception.CopyError(
        ...
        if transfer_proc.returncode != 0:
            raise EZDuplicator.lib.weresync.exception.CopyError(
                "Could not copy partition table to target device.", error)
    ...
    def _transfer_lvm(self, difference):
        ...
        for i in lvs:
            ...
            logging.debug("lvcreate command: " + str(command))
            with tempfile.TemporaryFile() as tmp:
                copy_proc = subprocess.Popen(
                    command, stdout=tmp, stderr=subprocess.PIPE)
                proc_output, error = copy_proc.communicate()
                tmp.seek(0)
                output = tmp.read()
                logging.debug("Output for " + i + ": " + str(output, "utf-8"))
    ...
    def format_partitions(self, ignore_errors=True, callback=None, lvm=False):
        ...
        :param ignore_errors: whether or not errors (such as a file-system
            ...
            try:
                ...
                if part_type is not None:
                    ...
                    logging.warning(
                        ...
                        "formatted.".format(i))
            except EZDuplicator.lib.weresync.exception.DeviceError as exe:
                if ignore_errors:
                    logging.warning(
    ...
    def transfer_partition_table(self, resize=True, callback=None, formatfs=False):
        ...
        :param formatfs: If true, then weresync will format each partition
        ...
        if target_size < source_size and not resize:
            logging.error("Target is smaller than source and resizing is disabled!")
        if source_type == "gpt":
            self._transfer_gpt(source_size - target_size, resize=resize)
        elif source_type == "msdos":
            self._transfer_msdos(source_size - target_size, resize=resize)
        for i in self.target.get_partitions():
            if self.target.mount_point(i) is not None:
                self.target.unmount_partition(i)
        if callback is not None:
    ...
    def partitions_valid(self, lvm=False):
        ...
        :raises: a :py:class:`~lib.weresync.exception.CopyError` if any part
                 invalid."""
        if lvm:
            source_manager = self.lvm_source
            target_manager = self.lvm_target
        ...
        for i in source_parts:
            source_file_system = source_manager.get_partition_file_system(i)
            target_file_system = target_manager.get_partition_file_system(i)
            if source_file_system != target_file_system and \
                    source_file_system not in SUPPORTED_PARTCLONE_FILESYSTEM_TYPES:
                raise EZDuplicator.lib.weresync.exception.CopyError(
            ...
            try:
                if source_manager.get_partition_used(i) > target_manager.get_partition_size(i):
                    raise EZDuplicator.lib.weresync.exception.CopyError(
                        ...
                        "corresponding partition on target drive. ({1}/{2})".format(i,
                                                                                    source_manager.get_partition_used(
                                                                                        i),
                                                                                    target_manager.get_partition_size(
                                                                                        i)))
            except EZDuplicator.lib.weresync.exception.DeviceError as ex:
                ...
                if "mount" in str(ex) or "swapspace" in str(ex):
                    logging.debug("Partition {0} couldn't be mounted. Bad FS type".format(i))
                else:
                    raise ex

        return True

    def _copy_fstab(self, mnt_source, mnt_target, excluded_partitions=None,
                    lvm=False):
    ...
    def _copy_files(self,
        ...
        for i in source_manager.get_partitions():
            ...
            try:
                ...
                logging.info("Starting rsync process for partition {0}.".format(
                    source_manager.device))
                ...
                logging.debug("Arguments = " + " ".join(command_args))
                ...
                def run_proc():
                    with subprocess.Popen(
                        ...
                        logging.debug("Errors for partition {0}:\n".format(i) +
                ...
                if callback is not None:
                    ...
                    logging.debug("Setting to finished")
                    callback(i, 1.0)
                else:
                    for val in run_proc():
                ...
                if ignore_failures:
                    ...
                    logging.warning("Error info.", exc_info=sys.exc_info())
                    if callback is not None:
                        callback(i, -1.0)
                else:
                    raise exe
    ...
    def make_bootable(self,
        ...
        if excluded_partitions is None:
            excluded_partitions = []
        logging.info("Using plugin: " + plugin_name)
        if callback is not None:
            callback(False)
        try:
            if plugin_name is not None:
                import EZDuplicator.lib.weresync.plugins as plugins
            ...
            try:
                ...
                logging.warning("Error copying fstab. Continuing anyway.")
                logging.debug("Info: ", exc_info=sys.exc_info())
            ...
            if self.lvm_source is not None:
                try:
                    ...
                    logging.warning("Error copying fstab on LVM. Continuing"
                                    " anyway.")
                    logging.debug("Info: ", exc_info=sys.exc_info())
            ...
            if plugin is not None:
                ...
                logging.warning("No bootloader plugin specified. Not installing"
                                "bootloader.")
            if callback is not None:
                callback(True)
        except DeviceError as ex:
            logging.warning("Error copying bootloader.")
            logging.debug("Info: ", exc_info=sys.exc_info())
