...
import EZDuplicator.lib.weresync.daemon.device as device
import EZDuplicator.lib.weresync.utils as utils
from EZDuplicator.lib.weresync.exception import (CopyError, DeviceError, UnsupportedDeviceError)
...
def create_new_vg_if_not_exists(lvm, name, target):
    ...
    if lvm_test.returncode == 5:
        ...
        logging.debug("PVs in LVM: " + result)
...
def copy_partitions(copier, part_callback=None, lvm=False):
    ...
    try:
        logging.info("Checking partition validity.")
        copier.partitions_valid(lvm)
        if part_callback is not None:
            part_callback(1.0)
            logging.info("Drives are compatible")
    except CopyError as ex:
        logging.exception(ex.message)
        logging.error("Partitions invalid! Copying drive partition table.")
        logging.warning("Drives are incompatible.")
        if lvm:
    ...
    def CopyDrive(self,
        ...
        **Note:** if using LVM, any uses of "partition" in the documentation
        ...
        if excluded_partitions is None:
            excluded_partitions = []
        logging.debug("Daemon starting clone.")
        ...
        try:
            ...
            copier = device.DeviceCopier(source_manager, target_manager, "/tmp/source", "/tmp/target")
            ...
            logging.info("Beginning to copy files.")
            copier.copy_files(
                mount_points[0],
                mount_points[1],
                excluded_partitions,
            ...
            try:
                ...
                logging.exception("Error making drive bootable. All files should be fine.")
                return ex
            logging.info("All done, enjoy your drive!")
            return "True"
        finally:
