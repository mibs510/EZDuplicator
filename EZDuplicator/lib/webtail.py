"""
Original Author: Santiago Coffey
Source code obtained from: https://gist.github.com/scoffey/1617014
...
class WebTailHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        self.http_headers = {} # noqa
        routes = {
            '/': lambda request: _STATIC_HTML, # noqa
            '/tail': self._get_tail
        }
        try:
            url = urlsplit(self.path)
            request = dict(parse_qsl(url.query))
    ...
    def _get_tail(self, request):
        ...
        ''' logging.info('tail(%r, %r, %r) returned offset %d and %d lines', self.filename, offset, limit, new_offset,
                     len(lines)) '''
    ...
    def tail(self, filename, offset=0, limit=None): # noqa
...
class WebTailServer(socketserver.ThreadingMixIn, HTTPServer):
    """ Web tail server.
    ...
    def handle_error(self, request, client_address):
        logging.exception('Error while processing request from %s:%d', *client_address)


def main(program, filename=None, port=7411, **kwargs): # noqa
